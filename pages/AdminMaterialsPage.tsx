  ...
  return (
    <div className="animate-fade-in pb-20 max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Manage Study Materials</h1>
        ...
        {isModalOpen && (
            <div className="fixed inset-0 bg-slate-900/90 z-[60] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
                <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50"><h3 className="font-bold text-lg dark:text-white">Manage Material</h3><button onClick={() => setIsModalOpen(false)} className="p-2 bg-white dark:bg-slate-700 rounded-full text-slate-500 shadow-sm">✕</button></div>
                    <form onSubmit={handleFormSubmit} className="p-6 space-y-4 overflow-y-auto">
                        <div className="flex gap-4 p-1 bg-slate-100 dark:bg-slate-700 rounded-lg mb-4"><button type="button" onClick={() => setIsAiMode(false)} className={`flex-1 py-2 rounded-md font-bold text-sm transition-all ${!isAiMode ? 'bg-white dark:bg-slate-800 text-indigo-600 shadow' : 'text-slate-500'}`}>Upload</button><button type="button" onClick={() => setIsAiMode(true)} className={`flex-1 py-2 rounded-md font-bold text-sm transition-all ${isAiMode ? 'bg-emerald-100 text-emerald-700 shadow' : 'text-slate-500'}`}>AI Generate</button></div>
