    ...
    <div className="space-y-8 animate-fade-in max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div onClick={() => navigate('/admin/approvals')} className={`bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden group cursor-pointer transition-all hover:shadow-md hover:-translate-y-1 ${isLibrarian ? 'hidden' : ''}`}>
                ...
                <h3 className="text-slate-500 font-bold uppercase text-[10px] tracking-widest mb-2">Pending Lost Items</h3>
                <p className="text-4xl font-bold text-amber-500">{stats.pendingLostFound}</p>
                {stats.pendingLostFound > 0 && <div className="absolute top-6 right-6 w-3 h-3 bg-amber-500 rounded-full animate-ping"></div>}
            </div>
        </div>
