...
export const AdminExecutivesPage: React.FC = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (editingItem) {
              await updateDoc(doc(db, 'executives', editingItem.id), payload);
          } else {
  ...
  return (
    <div className="animate-fade-in pb-20 max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-slate-900">Manage Executives</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {contentItems.map(item => (
                    <div key={item.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col group hover:border-indigo-200 transition-colors">
                        ...
                        <h4 className="font-bold text-slate-800 line-clamp-1 mb-1">{item.name}</h4>
                        <p className="text-xs text-indigo-500 font-bold uppercase line-clamp-1 mb-4 flex-1">{item.position}</p>
        ...
        {isModalOpen && (
            <div className="fixed inset-0 bg-slate-900/90 z-[60] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
                <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b flex justify-between items-center bg-slate-50"><h3 className="font-bold text-lg">Manage Executive</h3><button onClick={() => setIsModalOpen(false)} className="p-2 bg-white rounded-full text-slate-500 shadow-sm">✕</button></div>
                    <form onSubmit={handleFormSubmit} className="p-6 space-y-4 overflow-y-auto">
                        <div><label>Name</label><input className="w-full border p-3 rounded-xl" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} required /></div>
                        <div><label>Position</label><input className="w-full border p-3 rounded-xl" value={formData.position || ''} onChange={e => setFormData({...formData, position: e.target.value})} required /></div>
                        <div><label>Level</label><select className="w-full border p-3 rounded-xl" value={formData.level || '100'} onChange={e => setFormData({...formData, level: Number(e.target.value)})}>{LEVELS.filter(l => typeof l === 'number').map(l => <option key={l} value={l}>{l} Level</option>)}</select></div>
                        <div><label>WhatsApp</label><input className="w-full border p-3 rounded-xl" value={formData.whatsapp || ''} onChange={e => setFormData({...formData, whatsapp: e.target.value})} /></div>
                        <div><label>Email</label><input className="w-full border p-3 rounded-xl" value={formData.email || ''} onChange={e => setFormData({...formData, email: e.target.value})} /></div>
                        <div><label>Quote</label><textarea className="w-full border p-3 rounded-xl" rows={2} value={formData.quote || ''} onChange={e => setFormData({...formData, quote: e.target.value})} /></div>
                        <div><label>Photo</label><input type="file" className="w-full border p-3 rounded-xl" onChange={e => e.target.files && setFormFile(e.target.files[0])} accept="image/*" /></div>
