  ...
  return (
    <div className="animate-fade-in pb-20 max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Manage Executives</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {contentItems.map(item => (
                    <div key={item.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col group hover:border-indigo-200 dark:hover:border-indigo-600 transition-colors">
                        ...
                        <h4 className="font-bold text-slate-800 dark:text-white line-clamp-1 mb-1">{item.name}</h4>
        ...
        {isModalOpen && (
            <div className="fixed inset-0 bg-slate-900/90 z-[60] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
                <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50"><h3 className="font-bold text-lg dark:text-white">Manage Executive</h3><button onClick={() => setIsModalOpen(false)} className="p-2 bg-white dark:bg-slate-700 rounded-full text-slate-500 shadow-sm">✕</button></div>
                    <form onSubmit={handleFormSubmit} className="p-6 space-y-4 overflow-y-auto">
                        <div><label className="dark:text-slate-300">Name</label><input className={modalInputStyles} value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} required /></div>
                        <div><label className="dark:text-slate-300">Position</label><input className={modalInputStyles} value={formData.position || ''} onChange={e => setFormData({...formData, position: e.target.value})} required /></div>
                        <div><label className="dark:text-slate-300">Level</label><select className={`${modalInputStyles} bg-white`} value={formData.level || '100'} onChange={e => setFormData({...formData, level: Number(e.target.value)})}>{LEVELS.filter(l => typeof l === 'number').map(l => <option key={l} value={l}>{l} Level</option>)}</select></div>
                        <div><label className="dark:text-slate-300">WhatsApp</label><input className={modalInputStyles} value={formData.whatsapp || ''} onChange={e => setFormData({...formData, whatsapp: e.target.value})} /></div>
                        <div><label className="dark:text-slate-300">Email</label><input className={modalInputStyles} value={formData.email || ''} onChange={e => setFormData({...formData, email: e.target.value})} /></div>
                        <div><label className="dark:text-slate-300">Quote</label><textarea className={modalInputStyles} rows={2} value={formData.quote || ''} onChange={e => setFormData({...formData, quote: e.target.value})} /></div>
                        <div><label className="dark:text-slate-300">Photo</label><input type="file" className={`${modalInputStyles} file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100`} onChange={e => e.target.files && setFormFile(e.target.files[0])} accept="image/*" /></div>
