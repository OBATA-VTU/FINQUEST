...
export const AdminNewsPage: React.FC = () => {
  ...
  return (
    <div className="animate-fade-in pb-20 max-w-6xl mx-auto">
        <h1 className="text-2xl font-bold text-slate-900 mb-6">News & Announcements</h1>
        
        <div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100 mb-8">
            <h3 className="font-bold text-indigo-900 mb-4">Homepage HOD Message</h3>
        ...
        <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-bold text-slate-700">All Announcements</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {contentItems.map(item => (
                    <div key={item.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col group hover:border-indigo-200 transition-colors">
                        {item.imageUrl && <div className="w-full h-32 rounded-xl bg-slate-100 overflow-hidden mb-3"><img src={item.imageUrl} className="w-full h-full object-cover" alt="preview" /></div>}
                        <h4 className="font-bold text-slate-800 line-clamp-1 mb-1">{item.title}</h4>
                        <p className="text-xs text-slate-500 line-clamp-2 mb-4 flex-1">{item.content}</p>
        ...
        {isModalOpen && (
            <div className="fixed inset-0 bg-slate-900/90 z-[60] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
                <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b flex justify-between items-center bg-slate-50"><h3 className="font-bold text-lg">Manage Announcement</h3><button onClick={() => setIsModalOpen(false)} className="p-2 bg-white rounded-full text-slate-500 shadow-sm">✕</button></div>
                    <form onSubmit={handleFormSubmit} className="p-6 space-y-4 overflow-y-auto">
                        <div><label className="block text-xs font-bold uppercase mb-1">Title</label><input className="w-full border p-3 rounded-xl" value={formData.title || ''} onChange={e => setFormData({...formData, title: e.target.value})} required /></div>
                        <div><label className="block text-xs font-bold uppercase mb-1">Author</label><input className="w-full border p-3 rounded-xl" value={formData.author || ''} onChange={e => setFormData({...formData, author: e.target.value})} required /></div>
                        <div><label className="block text-xs font-bold uppercase mb-1">Content</label><textarea className="w-full border p-3 rounded-xl" rows={5} value={formData.content || ''} onChange={e => setFormData({...formData, content: e.target.value})} required /></div>
                        <div><label className="block text-xs font-bold uppercase mb-1">Image (Optional)</label><input type="file" className="w-full border p-3 rounded-xl" onChange={e => e.target.files && setFormFile(e.target.files[0])} accept="image/*" /></div>
