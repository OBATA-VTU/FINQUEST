...
export const AdminCommunityPage: React.FC = () => {
  ...
  const openModal = (item: any = null) => {
      setEditingItem(item);
      setFormData(item || { platform: 'WhatsApp' });
      setIsModalOpen(true);
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (editingItem) {
              await updateDoc(doc(db, 'groups', editingItem.id), payload);
          } else {
  ...
  return (
    <div className="animate-fade-in pb-20 max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Manage Community Groups</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {contentItems.map(item => (
                    <div key={item.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col group hover:border-indigo-200 dark:hover:border-indigo-600 transition-colors">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2 mb-4 flex-1">{item.description}</p>
        ...
        {isModalOpen && (
            <div className="fixed inset-0 bg-slate-900/90 z-[60] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
                <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50"><h3 className="font-bold text-lg dark:text-white">Manage Group</h3><button onClick={() => setIsModalOpen(false)} className="p-2 bg-white dark:bg-slate-700 rounded-full text-slate-500 shadow-sm">✕</button></div>
                    <form onSubmit={handleFormSubmit} className="p-6 space-y-4 overflow-y-auto">
                        ...
                        <div><label className="dark:text-slate-300">Platform</label><select className={`${modalInputStyles} bg-white`} value={formData.platform || 'WhatsApp'} onChange={e => setFormData({...formData, platform: e.target.value})}><option>WhatsApp</option><option>Telegram</option></select></div>
                        <div><label className="dark:text-slate-300">Link</label><input className={modalInputStyles} value={formData.link || ''} onChange={e => setFormData({...formData, link: e.target.value})} required /></div>
                        <div><label className="dark:text-slate-300">Description</label><textarea className={modalInputStyles} rows={3} value={formData.description || ''} onChange={e => setFormData({...formData, description: e.target.value})} required /></div>
