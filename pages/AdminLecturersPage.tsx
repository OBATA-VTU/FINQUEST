...
export const AdminLecturersPage: React.FC = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (editingItem) {
              await updateDoc(doc(db, 'lecturers', editingItem.id), payload);
          } else {
              await addDoc(collection(db, 'lecturers'), payload);
          }
  ...
  return (
    <div className="animate-fade-in pb-20 max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-slate-900">Manage Lecturers</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {contentItems.map(item => (
                    <div key={item.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col group hover:border-indigo-200 transition-colors">
                        ...
                        <p className="text-xs text-indigo-500 font-bold uppercase line-clamp-1 mb-4 flex-1">{item.title}</p>
        ...
        {isModalOpen && (
            <div className="fixed inset-0 bg-slate-900/90 z-[60] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
                <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b flex justify-between items-center bg-slate-50"><h3 className="font-bold text-lg">Manage Lecturer</h3><button onClick={() => setIsModalOpen(false)} className="p-2 bg-white rounded-full text-slate-500 shadow-sm">✕</button></div>
                    <form onSubmit={handleFormSubmit} className="p-6 space-y-4 overflow-y-auto">
                        ...
                        <div><label>Title</label><input className="w-full border p-3 rounded-xl" value={formData.title || ''} onChange={e => setFormData({...formData, title: e.target.value})} required /></div>
                        <div><label>Specialization</label><input className="w-full border p-3 rounded-xl" value={formData.specialization || ''} onChange={e => setFormData({...formData, specialization: e.target.value})} /></div>
