        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 md:p-8 animate-fade-in">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex flex-wrap justify-center items-center gap-2 sm:gap-4 mb-10 bg-slate-100 dark:bg-slate-900 p-2 rounded-full">
                    {LEVELS.filter(l => typeof l === 'number').map(l => <button key={l} onClick={() => setLevel(l as Level)} className={`px-6 sm:px-8 py-2.5 rounded-full text-sm font-black transition-all ${level === l ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white'}`}>{l}L</button>)}
                </div>
