...
const marketNewsEvents = [
    ...
    { text: "A bearish sentiment grips the market; investors are selling off.", effect: () => 0.96 },
];
...
export const TestPage: React.FC = () => {
  ...
  const [gameFeedback, setGameFeedback] = useState<{ status: 'correct' | 'wrong'; correctIndex: number} | null>(null);
  
  // CFO Challenge State
  const [cfoGameState, setCfoGameState] = useState({ scenarioIndex: 0, score: 0, feedback: null as CfoChoice | null });

  // Stock Market State
  const [stockGameState, setStockGameState] = useState({ day: 1, cash: 1000000, holdings: {} as Record<string, number>, news: '', prices: initialStocks.reduce((acc, s) => ({...acc, [s.ticker]: s.price}), {}) });
        ...
        timerRef.current = window.setInterval(() => {
            setTimeLeft((prev) => {
                if (prev <= 1) {
                    ...
                    if (mode === 'rapid_fire') handleGameOver(); else finishTest();
                    return 0;
                }
                return prev - 1;
            });
  ...
  const handleStockTrade = () => {
      ...
      if (type === 'buy') {
          ...
          setStockGameState(prev => ({
              ...prev,
              cash: prev.cash - cost,
              holdings: { ...prev.holdings, [stock.ticker]: (prev.holdings[stock.ticker] || 0) + tradeAmount }
          }));
      } else { // sell
          ...
          setStockGameState(prev => ({
              ...
              holdings: { ...prev.holdings, [stock.ticker]: currentHolding - tradeAmount }
          }));
      }
  ...
  const endStockDay = () => {
    ...
    initialStocks.forEach(stock => {
        ...
        newPrices[stock.ticker] = Math.max(1.00, newPrices[stock.ticker] * (1 + baseChange) * newsEffect);
    });
  ...
  const renderResult = () => { 
    ...
    if (['topic', 'mock'].includes(mode)) {
        ...
        // FIX: Replaced Object.entries with Object.keys for type safety.
        // This resolves issues where `shares` was inferred as 'unknown', causing arithmetic errors.
  ...
  const renderRapidFireGame = () => { if (questions.length === 0) return null; return ( <div className="min-h-screen bg-slate-900 text-white flex flex-col p-8"><header className="flex justify-between items-center mb-8"><h2 className="text-2xl font-bold font-serif text-indigo-300">Rapid Fire</h2><div className="flex items-center gap-4"><div className="flex gap-1 text-2xl">{Array(gameLives).fill(0).map((_, i) => <span key={i} className="text-rose-500">♥</span>)}{Array(3-gameLives).fill(0).map((_, i) => <span key={i} className="text-rose-900">♥</span>)}</div><div className="px-4 py-2 bg-amber-400 text-slate-900 rounded font-bold">{gameScore}</div></div></header><main className="flex-1 flex flex-col items-center justify-center text-center"><div className="relative w-24 h-24 mb-6"><svg className="w-full h-full transform -rotate-90"><circle cx="48" cy="48" r="44" stroke="currentColor" strokeWidth="8" fill="transparent" className="text-indigo-800" /><circle cx="48" cy="48" r="44" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray={276.4} strokeDashoffset={276.4 - (276.4 * timeLeft) / 30} className="text-indigo-400" /></svg><span className="absolute inset-0 flex items-center justify-center text-3xl font-mono">{timeLeft}</span></div><h3 className="text-3xl font-bold max-w-3xl mb-12">{questions[currentQuestionIndex].text}</h3><div className="grid grid-cols-2 gap-4 w-full max-w-3xl">{questions[currentQuestionIndex].options.map((opt, i) => (<button key={i} onClick={() => handleRapidFireAnswer(i)} disabled={!!gameFeedback} className={`p-4 rounded-xl text-lg font-semibold text-left border-4 ${gameFeedback && gameFeedback.correctIndex === i ? 'bg-emerald-500' : ''} ${gameFeedback?.status === 'wrong' && userAnswers[currentQuestionIndex] === i ? 'bg-rose-600' : ''} ${!gameFeedback ? 'bg-slate-800' : 'border-transparent'}`}>{opt}</button>))}</div></main></div>); };
  const renderCfoChallengeGame = () => { const scenario = cfoScenarios[cfoGameState.scenarioIndex]; return ( <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-8 text-center"><h2 className="text-indigo-300 font-bold uppercase tracking-widest text-sm mb-4">CFO Challenge - Scenario {cfoGameState.scenarioIndex + 1}/{cfoScenarios.length}</h2><p className="text-3xl font-serif font-bold max-w-3xl mb-12">{scenario.prompt}</p><div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-4xl">{scenario.choices.map((choice, i) => (<button key={i} onClick={() => handleCfoChoice(choice)} disabled={!!cfoGameState.feedback} className="p-6 bg-slate-800 rounded-xl text-left hover:bg-indigo-800 transition disabled:opacity-70">{choice.text}</button>))}</div>{cfoGameState.feedback && <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4"><div className="bg-white text-slate-900 p-8 rounded-2xl max-w-lg animate-fade-in-up"><h3 className={`text-2xl font-bold mb-4 ${cfoGameState.feedback.score > 5 ? 'text-emerald-600' : 'text-amber-600'}`}>Consequence</h3><p className="mb-6">{cfoGameState.feedback.feedback}</p><button onClick={nextCfoScenario} className="w-full py-3 bg-indigo-600 text-white font-bold rounded-lg">Next Scenario</button></div></div>}</div> ); };
  const renderStockMarketGame = () => { 
    // FIX: Replaced Object.entries with Object.keys for type safety.
    // This resolves issues where `shares` was inferred as 'unknown', causing arithmetic errors.
    const portfolioValue = stockGameState.cash + Object.keys(stockGameState.holdings).reduce((acc, ticker) => acc + ((stockGameState.holdings[ticker] || 0) * (stockGameState.prices[ticker] || 0)), 0); 
    return ( <div className="min-h-screen bg-slate-950 text-white p-6"><header className="flex justify-between items-center mb-6"><h2 className="text-2xl font-serif font-bold text-indigo-300">FINSA Stock Exchange</h2><div className="text-right"><p className="text-xs uppercase text-slate-400">Day {stockGameState.day}/10</p><p className="text-2xl font-bold">{formatNaira(portfolioValue)}</p></div></header><div className="bg-slate-800/50 p-3 rounded-xl text-center text-sm mb-6 animate-pulse">📢 {stockGameState.news}</div><div className="grid grid-cols-1 lg:grid-cols-3 gap-6"><div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">{initialStocks.map(stock => (<div key={stock.ticker} className="bg-slate-800 p-4 rounded-xl border border-slate-700"><div className="flex justify-between items-center mb-2"><h3 className="font-bold">{stock.ticker}</h3><p className="font-mono text-lg">{formatNaira(stockGameState.prices[stock.ticker])}</p></div><div className="flex gap-2"><button onClick={() => setTradeModal({type: 'buy', stock})} className="flex-1 py-2 bg-emerald-600 rounded text-xs font-bold">Buy</button><button onClick={() => setTradeModal({type: 'sell', stock})} className="flex-1 py-2 bg-rose-600 rounded text-xs font-bold">Sell</button></div></div>))}</div><div className="bg-slate-800 p-6 rounded-2xl flex flex-col"><h3 className="font-bold mb-4">Your Portfolio</h3><p className="text-3xl font-bold text-emerald-400 mb-4">{formatNaira(stockGameState.cash)}</p><div className="space-y-2 flex-1 overflow-y-auto mb-4">{Object.entries(stockGameState.holdings).map(([ticker, shares]) => shares > 0 && (<div key={ticker} className="flex justify-between text-sm"><span className="font-bold">{ticker}: {shares}</span><span>{formatNaira(shares * stockGameState.prices[ticker])}</span></div>))}</div><button onClick={endStockDay} className="w-full py-3 bg-indigo-600 rounded-lg font-bold mt-auto">End Day</button></div></div>{tradeModal && <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4"><div className="bg-slate-800 p-8 rounded-2xl w-full max-w-sm"><h3 className="text-xl font-bold mb-4 capitalize">{tradeModal.type} {tradeModal.stock.ticker}</h3><input type="number" value={tradeAmount} onChange={e => setTradeAmount(parseInt(e.target.value) || 0)} className="w-full p-3 rounded bg-slate-700 text-white mb-4" placeholder="Number of shares" /><div className="flex gap-2"><button onClick={() => setTradeModal(null)} className="flex-1 py-2 border rounded">Cancel</button><button onClick={handleStockTrade} className="flex-1 py-2 bg-indigo-600 rounded font-bold">Confirm</button></div></div></div>}</div>); };

  const GameCard = ({ title, desc, icon, onClick, mode }: any) => (
      <div onClick={() => onClick(mode)} className="group relative bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all border border-slate-200 dark:border-slate-700 hover:-translate-y-2 text-left cursor-pointer overflow-hidden flex flex-col">
          <div className="absolute top-0 right-0 p-4 text-slate-100 dark:text-slate-800 text-8xl opacity-50 group-hover:opacity-100 group-hover:scale-125 transition-all duration-300 group-hover:rotate-6">{icon}</div>
          <div className="relative z-10 flex-1 flex flex-col">
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-3">{title}</h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm flex-1">{desc}</p>
            <div className="mt-6 text-sm font-bold text-indigo-600 dark:text-indigo-400 group-hover:underline">Play Now &rarr;</div>
          </div>
      </div>
  );
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 py-16 px-4 animate-fade-in">
            <div className="container mx-auto max-w-7xl">
                <div className="text-center mb-12">
                    <h1 className="text-5xl font-serif font-bold text-slate-900 dark:text-white mb-4">FINQUEST Arcade</h1>
                    <p className="text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">Apply your knowledge in our collection of interactive finance games.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <GameCard title="Rapid Fire Quiz" desc="A fast-paced survival quiz. Answer questions against the clock, three strikes and you're out!" icon="🕹️" onClick={startExam} mode="rapid_fire" />
                    <GameCard title="Naija Finance Timeline" desc="Test your knowledge of Nigeria's key financial and economic events. Place historical moments in their correct time periods." icon="🇳🇬" onClick={startExam} mode="timeline" />
                    <GameCard title="FINSA Stock Exchange" desc="Manage a virtual ₦1M portfolio. Buy and sell real Nigerian stocks over 10 simulated days, reacting to dynamic market news." icon="📈" onClick={startExam} mode="stock_market" />
                    <GameCard title="The CFO Challenge" desc="Step into the shoes of a CFO and make tough corporate finance decisions. Your choices have real consequences." icon="👔" onClick={startExam} mode="cfo_challenge" />
                </div>
                 <div className="text-center mt-20">
                    <h2 className="text-2xl font-serif font-bold text-slate-800 dark:text-white mb-4">Or, practice with Standard CBT</h2>
                    <div className="flex justify-center gap-6">
                        <button onClick={() => { setMode('topic'); setStage('setup'); }} className="px-8 py-4 bg-white dark:bg-slate-800 rounded-xl shadow-md border">Topic Mastery</button>
                        <button onClick={() => { setMode('mock'); setStage('setup'); }} className="px-8 py-4 bg-white dark:bg-slate-800 rounded-xl shadow-md border">Standard Mock</button>
                    </div>
                </div>
            </div>
        </div>
