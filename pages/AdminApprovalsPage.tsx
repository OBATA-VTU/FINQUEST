            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {pendingItems.map(item => (
                    <div key={item.id} className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col hover:border-indigo-300 transition-colors">
                        ...
                        <p className="text-[10px] font-black uppercase text-slate-400 mb-1 tracking-widest">{item.uploadedByName || 'Member'}</p>
                        <p className="text-[9px] font-medium text-slate-400 mb-4 font-mono">{item.uploadedByEmail || (item.uploadedBy ? `UID: ${item.uploadedBy.slice(-8)}` : 'UNKNOWN')}</p>
