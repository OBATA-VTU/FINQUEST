    ...
    <div className="animate-fade-in max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-6">
            <div>
                ...
                <p className="text-sm text-slate-500">Review uploads before they go live.</p>
            </div>
            <span className="bg-rose-100 text-rose-600 px-3 py-1 rounded-full text-xs font-bold">{pendingItems.length} Pending</span>
        </div>
                    ...
                    <div key={item.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col hover:border-indigo-300 transition-colors">
                        ...
                        <div className="mt-auto pt-4 border-t border-slate-50 flex gap-2">
                            <a 
                                ...
                                target="_blank" 
                                rel="noreferrer" 
                                className="flex-1 py-2 text-center text-xs font-bold text-slate-600 bg-slate-50 rounded hover:bg-slate-100 transition"
                            >
                                Preview
                            </a>
                            <button 
                                onClick={() => handleApproval(item, true)} 
                                className="flex-1 py-2 text-center text-xs font-bold text-white bg-emerald-500 rounded hover:bg-emerald-600 transition"
                            >
                                Approve
                            </button>
                            <button 
                                onClick={() => handleApproval(item, false)} 
                                className="px-3 py-2 text-center text-xs font-bold text-rose-500 bg-rose-50 rounded hover:bg-rose-100 transition"
                            >
                                Reject
                            </button>
                        </div>
