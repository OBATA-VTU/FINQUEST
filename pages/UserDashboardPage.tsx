  ...
  useEffect(() => {
      const fetchData = async () => {
          ...
          try {
              ...
              // Fetch Recent News (Limit 2)
              const newsQ = query(collection(db, 'announcements'), orderBy('date', 'desc'), limit(2));
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pb-20 font-sans transition-colors">
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                  <div>
                      <div className="flex items-center gap-2 mb-2">
                          <span className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Student Dashboard</span>
                          <span className="bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-[10px] font-bold px-2 py-0.5 rounded-full">{user.level} Level</span>
                      </div>
                      <h1 className="text-3xl md:text-4xl font-serif font-bold text-slate-900 dark:text-white mb-2">
                          {greeting}, {user.name.split(' ')[0]}
                      </h1>
                      <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700/50 px-3 py-1.5 rounded-lg w-fit">
                          <span className="font-mono text-sm font-bold text-indigo-600 dark:text-indigo-400">@{user.username}</span>
                          <VerificationBadge role={user.role} isVerified={user.isVerified} className="w-4 h-4" />
                      </div>
                  </div>
                  
                  <div className="w-full md:w-auto max-w-lg bg-indigo-50 dark:bg-indigo-900/20 border-l-4 border-indigo-500 p-4 rounded-r-lg">
                      <p className="text-indigo-800 dark:text-indigo-200 text-sm italic font-medium">"{quote}"</p>
                  </div>
              </div>
          </div>
      </div>
      ...
      <div className="container mx-auto px-4 max-w-6xl -mt-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {/* LEFT COLUMN - ACTIONS & HISTORY (2/3) */}
