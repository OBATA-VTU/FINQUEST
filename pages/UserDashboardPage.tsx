  ...
  useEffect(() => {
      const fetchData = async () => {
          ...
          try {
              ...
              const newsSnap = await getDocs(query(collection(db, 'announcements'), limit(3)));
              let news = newsSnap.docs.map(d => ({ id: d.id, ...d.data() } as Announcement));
              setRecentNews(news);

              // FIXED RECOMMENDATION LOGIC: Strictly match Level OR General tags
              ...
              // Recommend only user level OR items tagged "General/Other"
              ...
              <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                  <div>
                      <div className="flex items-center gap-3 mb-6">
                         ...
                         <div>
                            <p className="text-indigo-300 text-[10px] font-black uppercase tracking-[0.2em]">{user.level} Level Student</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="md:col-span-2 space-y-8">
                  <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm">
                      <h3 className="font-black uppercase text-[10px] text-indigo-500 tracking-widest mb-8">Relevant for Your Level ({user.level}L)</h3>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                          {recommendedQuestions.length > 0 ? recommendedQuestions.map(q => (
                              <div key={q.id} onClick={() => navigate('/questions')} className="p-6 bg-slate-50 dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 cursor-pointer hover:scale-[1.02] transition-all">
                                  ...
                                  <p className="text-[10px] text-slate-400">{q.year} Question</p>
                              </div>
                          )) : <div className="col-span-3 text-center py-6 text-slate-400 italic">No specific level resources found yet.</div>}
                      </div>
                  </div>
                  <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm">
                      <h3 className="font-black uppercase text-[10px] text-rose-500 tracking-widest mb-8">News Feed</h3>
                          ...
                          {recentNews.map(news => (
                              <div key={news.id} onClick={() => navigate('/announcements')} className="flex items-center gap-6 group cursor-pointer">
                                  <div className="w-12 h-12 bg-slate-50 dark:bg-slate-800 rounded-2xl flex flex-col items-center justify-center font-black text-slate-400 shrink-0">
                                      ...
                                      <span className="text-[8px] uppercase">{new Date(news.date).toLocaleDateString(undefined, {month:'short'})}</span>
                                  </div>
                                  <div>
              ...
              <div className="space-y-8">
                  <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm">
                      <h3 className="font-black uppercase text-[10px] text-emerald-500 tracking-widest mb-8">Test Summary</h3>
                      <div className="space-y-6">
                          {recentTests.map(test => (
                              <div key={test.id} className="flex justify-between items-center">
                                  <div>
                                      <p className="font-bold text-slate-800 dark:text-white text-sm">CBT Practice</p>
                          ...
                          {recentTests.length === 0 && <p className="text-center text-slate-400 text-xs py-4">Take a test to see stats.</p>}
                      </div>
                      <button onClick={() => navigate('/test')} className="w-full mt-10 py-4 bg-indigo-600 text-white font-black rounded-2xl uppercase tracking-widest text-[10px] shadow-xl shadow-indigo-500/20">Start Mock Session</button>
                  </div>
              </div>
          </div>
      </div>
