...
export const Footer: React.FC = () => {
  ...
  const [socials, setSocials] = useState({
      facebook: 'https://facebook.com/groups/8173545352661193/',
      twitter: 'https://x.com/FINSA_AAUA',
      instagram: '',
      whatsapp: 'https://whatsapp.com/channel/0029VbC0FW23QxS7OqFNcP0q'
  });
      ...
      const fetchContent = async () => {
          try {
              ...
              if (socDoc.exists()) {
                  ...
                  // Merge fetched data over the new defaults.
                  // This ensures DB values take priority, but new defaults are used if DB fields are missing.
    ...
    <footer className="relative bg-slate-950 text-slate-300 overflow-hidden border-t border-slate-800 mt-auto">
      ...
      <div className="container mx-auto px-6 py-16 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-3">
            ...
            <ul className="space-y-3">
                {auth?.user ? (
                    ...
                    <>
                        <li><Link to="/login" className="text-sm hover:text-indigo-400 transition-colors flex items-center gap-2 group"><span className="w-1 h-1 bg-slate-600 group-hover:bg-indigo-400 rounded-full transition-colors"></span>Login / Register</Link></li>
          ...
          {/* CONTACT (3 cols) */}
          <div className="lg:col-span-3">
            <h3 className="font-bold text-white mb-6 uppercase text-xs tracking-widest border-l-2 border-indigo-500 pl-3">Contact Support</h3>
            <address className="not-italic space-y-4">
              <div className="flex items-start gap-3">
                  ...
                  <p className="text-sm text-slate-400">Department of Finance,<br/>Adekunle Ajasin University.</p>
              </div>
              
              <div className="flex flex-col gap-3 pt-2">
                  {/* WhatsApp is now the primary/only recommended contact method per user request */}
                  <a href="https://wa.me/2348142452729" className="group flex items-center gap-3 bg-slate-900 border border-slate-800 p-3 rounded-lg hover:border-green-600 hover:bg-slate-800 transition-all">
                      ...
                      <div className="flex flex-col">
                          <span className="text-xs text-slate-500 font-bold uppercase tracking-wider">PRO Support</span>
        ...
        <div className="mt-16 pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-slate-500">
          <div className="flex flex-col md:flex-row items-center gap-1 md:gap-4">
              ...
              <p className="flex items-center gap-1">
                  Developed by <span className="text-indigo-400 font-bold bg-indigo-900/20 px-2 py-0.5 rounded border border-indigo-900/50">OBA - PRO '25/26</span>
              </p>
          </div>
