...
export const Sidebar: React.FC<SidebarProps> = ({ onNavigate, currentPage, isOpen, onClose }) => {
  ...
  const NavItem = ({ page, label, icon }: { page: Page; label: string; icon: React.ReactNode }) => (
    ...
    >
      <div className={`${currentPage === page ? 'text-white' : 'text-slate-400 group-hover:text-indigo-600'}`}>
        {icon}
      </div>
