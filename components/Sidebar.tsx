...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const NavItem = ({ to, label, icon }: { to: string; label: string; icon: React.ReactNode }) => (
    <NavLink
      to={to}
      onClick={onClose}
      className={({ isActive }) => `w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 group mb-1 ${
        isActive
    ...
    <>
        ...
        >
          ...
          <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-700">
            ...
            <div className="mt-6 mb-2 px-4 text-xs font-bold text-indigo-400 uppercase tracking-widest">Resources</div>
            
            {/* RESTRICTED: Past Questions */}
            {auth?.user && (
                <NavItem 
                    to="/questions" 
            ...
            <div className="mt-6 mb-2 px-4 text-xs font-bold text-indigo-400 uppercase tracking-widest">Department</div>
             <NavItem 
                to="/announcements" 
                label="News & Updates" 
